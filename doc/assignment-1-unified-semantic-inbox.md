...
flowchart LR
    ...
    D -->|Duplicate| E[Return 200 and drop]
    D -->|New Event| F[Publish to Message Queue]
    F --> G[Intent Engine Workers]
    G --> H[Identity Resolver]
    H --> I[(MongoDB user_identity_map)]
    G --> J[Context Reader]
    J --> K[(MongoDB message_buckets)]
    G --> L[Semantic Cache Lookup]
    L -->|Cache Hit| M[Intent Decision]
    L -->|Cache Miss| N[LLM Classifier]
    N --> O[Cache Write]
    O --> M
    M --> P[Persist Result and Message]
    P --> K
    M --> Q[Downstream Router]
    C --> R[Fast ACK 200]
```
